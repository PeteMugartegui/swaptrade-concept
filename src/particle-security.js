...
void main(){float motion=1.0-uReducedMotion;float bob=sin(uTime*(.52+uHero*.12))*(.012+uHero*.005)*motion;float pulse=1.0+sin(uTime*.38)*(.004+uHero*.002)*motion;vec2 p=aPosition*pulse+vec2(sin(uTime*.31)*.004*motion,bob);float group=floor(aSeed*6.0);float shardAngle=group*1.0472+.35;float breathe=(.004+.004*sin(uTime*.72+group))*uHero*motion;p+=vec2(cos(shardAngle),sin(shardAngle))*breathe;float depth=(fract(aSeed*6.0)-.5)*.22*uHero;float turn=(-.07+sin(uTime*.24)*.018*motion)*uHero;float rx=p.x*cos(turn)+depth*sin(turn);depth=-p.x*sin(turn)+depth*cos(turn);p.x=rx;vec2 projected=vec2(p.x/uAspect,p.y);vec2 delta=projected-uPointer;float influence=smoothstep(.5+uHero*.1,0.0,length(delta))*uPointerStrength;float randomAngle=fract(aSeed*37.17)*6.28318+sin(uTime*.7+aSeed*19.0)*.28;float randomForce=.14+fract(aSeed*91.7)*.25;vec2 burst=vec2(cos(randomAngle)*uAspect,sin(randomAngle))*influence*randomForce;vec2 repel=normalize(delta+vec2(.0001))*influence*.08;float wake=sin(length(delta)*24.0-uTime*5.0+aSeed*8.0)*influence*.018;p+=burst+vec2(repel.x*uAspect,repel.y)+normalize(delta+vec2(.0001))*wake;depth+=influence*(fract(aSeed*53.1)-.2)*.85;float perspective=1.0/(1.0-depth*.2);p*=perspective;gl_Position=vec4(p.x/uAspect,p.y,0,1);gl_PointSize=(1.35+fract(aSeed*19.3)*1.45+influence*(1.0+fract(aSeed*11.2)*2.2)+uHero*.3)*perspective;vSeed=aSeed;vAlpha=(.4+fract(aSeed*13.7)*.58)*(1.0-influence*.16);}`;
